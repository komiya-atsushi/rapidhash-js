...
interface RapidhashFunc {
  (message: Buffer, len: number, seed: bigint): bigint;
}
...
function compile(behaviour: 'fast' | 'protected'): [RapidhashFunc, bigint] {
  ...
  const rapidhash: RapidhashFunc = (message, len, seed) =>
    result.symbols._rapidhash_withSeed(ptr(message), len, seed);

  return [rapidhash, result.symbols.default_seed()];
}

function generateTestVectors1(
  rapidhash: RapidhashFunc,
  seeds: bigint[]
  ...
  return seeds.flatMap(seed =>
    testMessages
      ...
      .map(
        ([message, buf]) =>
          [message, seed, rapidhash(buf, buf.length, seed)] as const
      )
  );
}
...
function generateTestVectors2(rapidhash: RapidhashFunc, seeds: bigint[]) {
  ...
  const testLongMessage = _testLongMessage.repeat(
    Math.ceil((2 ** 13 + 1) / _testLongMessage.length)
  );
  ...
  for (let i = 1; i < buffer.length; i++) {
    ...
    result.push(
      ...seeds.map(seed => [i, seed, rapidhash(buffer, i, seed)] as const)
    );
  }
...
function generateTestVectors(
  ...
  const {longMessage, testVectors: testVectors2} = generateTestVectors2(
    rapidhash,
    [defaultSeed]
  );

  const writeStream = fs.createWriteStream(
    `generated/test_vector_${behaviour}.ts`
  );
  ...
  writeStream.write(
    util.inspect(testVectors1, {depth: null, maxArrayLength: Infinity})
  );
  writeStream.write(';\n\n');
  ...
  writeStream.write(
    util.inspect(testVectors2, {depth: null, maxArrayLength: Infinity})
  );
  writeStream.write(';\n');
...
function main() {
  const revision = child_process
    .execSync("git -C src/rapidhash show --format='%H' --no-patch")
    .toString()
    .trim();
