...
function rapidhashMicro_v3_0_internal(
  key: DataView,
  seed: bigint,
  secret: RapidSecret,
  rapid_mix: RapidMix,
...
/**
 * Calculate a 64-bit hash value of the given message using the Micro variant.
 * This variant uses 80-byte loop blocks for optimized performance characteristics.
 *
 ...
 */
export function rapidhashMicro(message: string | Uint8Array | DataView, options?: Partial<RapidhashOptions>): bigint {
  ...
  return rapidhashMicro_v3_0_internal(toDataView(message), seed, rapid_secret, rapid_mix, rapidhash_epilogue);
}

/**
 * Calculate a 64-bit hash value of the given message using the Micro variant.
 * This function is equivalent to rapidhashMicro() with options.rapidMumBehaviour set to 'fast'.
 *
 * @param message {string|Uint8Array|DataView} The message to be hashed.
 * @param options Options for modifying the hash calculation.
 * @param options.seed {bigint} 64-bit unsigned seed value.
 */
export function rapidhashMicro_fast(
  ...
  return rapidhashMicro_v3_0_internal(
    toDataView(message),
    seed,
    rapid_secret,
    rapid_mix_fast,
...
/**
 * Calculate a 64-bit hash value of the given message using the Micro variant.
 * This function is equivalent to rapidhashMicro() with options.rapidMumBehaviour set to 'protected'.
 *
 * @param message {string|Uint8Array|DataView} The message to be hashed.
 * @param options Options for modifying the hash calculation.
 * @param options.seed {bigint} 64-bit unsigned seed value.
 */
export function rapidhashMicro_protected(
  message: string | Uint8Array | DataView,
  options?: Partial<Omit<RapidhashOptions, 'rapidMumBehaviour'>>,
): bigint {
  const {seed} = validateOptions(options, RAPID_SEED);
